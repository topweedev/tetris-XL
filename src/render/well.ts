...
export function createWell(camera?: THREE.Camera): THREE.Group {
  ...
  if (camera) { const cameraNear: number = ('near' in camera ? camera.near : 0) as number; bakeZWallNear(camera, walls); for (const wall of walls) { const position = wall.geometry.getAttribute('position'); const colors = new Float32Array(position.count * 3); for (let i = 0; i < position.count; i += 1) { const point = new THREE.Vector3().fromBufferAttribute(position, i).applyMatrix4(wall.matrixWorld).applyMatrix4(camera.matrixWorldInverse); const distance = Math.abs(point.z); const fade = THREE.MathUtils.smoothstep(distance, cameraNear + FADE_NEAR_OFFSET, cameraNear + FADE_FAR_OFFSET) * alpha; colors.set([WALL_COLOR.r * fade, WALL_COLOR.g * fade, WALL_COLOR.b * fade], i * 3); } wall.geometry.setAttribute('color', new THREE.BufferAttribute(colors, 3)); } }
  return group;
}
