...
- 鍵盤為主：方向鍵 X/Y 平移、`Q/E/W/S/A/D` 對應三軸旋轉、`Space` 硬降、`Shift` 軟降（per ADR-0004 rev.2 §2.4）、`C` Hold（per ADR-0005 rev.2 §2.1）。
...
依 ADR-0004 rev.2 §5 與 ADR-0005 rev.2 §5 明訂之修訂條款，收斂 §2.6 之鍵位語意；本 rev 為 upstream-driven delta：

- **§2.6 鍵位對應**：
  - `Shift`：由「保留 (hold)」正式改為 **軟降 (SoftDrop)**，per ADR-0004 rev.2 §2.4 spike keymap（`LShift` = `GameAction.SoftDrop = 30`）。原「保留 (hold)」設計因 ADR-0004 §2.4 之 4-方位平移需求已改用 `LShift` 承載 SoftDrop 語意。
  - `C`：新增 Hold 綁定，per ADR-0005 rev.2 §2.1（`HOLD_KEY_CODE = "KeyC"`；`GameAction.Hold = 40`）。ADR-0004 §5 曾 punt「試探 `Tab` 或 `C`」，ADR-0005 §2.1 已決策為 `C`（`Tab` 有 focus-navigation 副作用）。

**verdict**：本 rev 屬 upstream-driven delta，內容 100% 由已 Accepted 之 ADR-0004 rev.2、ADR-0005 rev.2 決策推出；不觸發新 LA4 / LA5 review round。

**status**：Accepted 維持不變（rev.4 為 final round 之定案；本 rev 僅同步下游鍵位語意，不動 §2.1 / §2.4 / §2.7 之演算法或不變式）。
