...
- 狀態：Accepted（rev.3）
- 日期：2026-07-22
- 修訂：2026-07-22（rev.2 — 依 LA5 review 補齊 B1–B3 與部分 S/N 項目；rev.3 — 自我 review 清理 3 nit）
- 決策者：專案發起團隊
- 相關文件：README.md
...
```
BOOT → READY → SPAWN → FALLING → GROUNDED → LOCKED → CLEARING → SPAWN
                                                 ↘ GAME_OVER
```
...
| `GROUNDED` | Piece 底面某 cell 下一格為場地或 `z=0` | (a) `lockDelay` 到 → `LOCKED`；(b) 玩家移動/旋轉成功 → 續留 `GROUNDED`，計數重置；(c) 硬降 → 立即 `LOCKED` |
| `LOCKED` | `GROUNDED` 逾時或硬降 | Piece cell 寫入場地 → `CLEARING` |
...
1. **Spawn blocked**（含 kick 嘗試）：`SPAWN` 生成新 piece 時執行碰撞檢查；若原位失敗，依 §2.2 wall/floor-kick 偏移列表嘗試位移；**全部失敗**即 game over。（即 well 頂已無合法容納空間。）
...
1. 方塊鎖定後，掃描其涵蓋之所有 Z 層（僅這些層可能因本次 lock 變滿）。
2. 對每一層檢查 25 格是否皆非零。
3. 收集所有滿層（`clearedLayers: number[]`），一次性以動畫（發光 → 收縮 → 消失，約 300 ms）呈現。
...
5. **不做連鎖再掃描**：每次 lock 僅結算一輪清除。下移後即使產生新的滿層，也保留至下次 lock 再判定。此決策優先於連鎖華麗度以換取行為可預測與 FSM 簡單。
...
- **B1 → §2.4.3 / §2.4.4**：新增 Game Over 三條件與 Spawn 定位規則。
