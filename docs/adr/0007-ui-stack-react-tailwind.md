...
- 狀態：Accepted（rev.2；rev.1 為 initial draft，LA8 review round 1 verdict NEEDS_CHANGES 後修畢）
- 日期：2026-07-22
- 決策者：專案發起團隊
- 相關文件：
...
- ✅ `Suspense`：Replay browser 之 IndexedDB 讀取邊界；spike 期以 `React.lazy` 之 lazy resolution 或 promise wrapper 作為 Suspense trigger
- ❌ `use()` hook：屬 React 19（截至本 ADR 之 rev.1 尚未 stable release）；spike 期**不**採用。Rev.2 candidate 若升 React 19 再議
...
},
{
  ...
  rules: {
    'no-restricted-imports': ['error', {
      patterns: [{
        group: ['**/engine/**', '@engine/**'],
        message: 'ui/screens|hud|components must go through ui/bridge/* (not engine/ directly)',
      }],
    }],
  },
},
...
- `bundle-size` job：於 PR 加 `pnpm build --report` 之 dist 大小 diff comment；fail 條件為任一 chunk 之 gzip 大小 > `target × 1.20`（見 §2.6 N3 clarification）
- `css-purge-check`：驗證 `dist/styles.css` 不含未使用之 utility class（Tailwind JIT 應保證，但加 belt-and-braces）
- `csp-inline-style-grep`：於 `src/ui/**/*.tsx` grep `style={` → 命中則 fail（強制執行 §2.7 之 React 側規則）
...
- `tailwind-dynamic-class-grep`（S3 fix）：於 `src/ui/**/*.tsx` grep `className={\`[^}]*\${` template-literal 動態組合 → 命中則 warn（若真需要則於 `tailwind.config.ts` safelist）
