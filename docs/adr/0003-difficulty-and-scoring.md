...
- **實作**：`const GRAVITY_TABLE: readonly u8[] = [60, 48, 37, 28, 21, 16, 12, 9, 7, 5, 4, 3, 2, 1, 1, 1, 1, 1, 1, 1];`（長度 20，索引 = `level - 1`）
...
**實作**：`const LOCK_DELAY_TABLE_MS: readonly u16[] = [500,500,500,500,500,500,500,500,500,500, 450,400,350,300, 250,250,250,250,250,250];`
...
1. 每輪 bag 抽 7 個 piece，依當前 level 對應的桶權重加權
2. Bag 清空後重抽下一 bag；bag 邊界處禁止同 typeId 連續超過 3 次（既有 anti-repeat 規則）
3. **關卡變化不影響進行中的 bag**：升級發生於一 bag 抽完後才生效，避免同 bag 內權重跳變

**實作**：`const BAG_WEIGHTS: readonly [u8, u8, u8, u8][] = [ /* 20 rows */ ];`
...
- `BASE_LINE_SCORE[1..4]` 嚴格遞增（1500 > 700 > 300 > 100）
